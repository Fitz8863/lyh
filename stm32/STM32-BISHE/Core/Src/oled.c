...
/**
 * @brief  设置OLED显示方向
 * @param  flip: 0-正常方向, 1-上下翻转
 * @retval 无
 */
void OLED_SetFlip(uint8_t flip)
{
    if (flip)
    {
        // 上下翻转模式
        WriteCmd(0xC0); // COM输出扫描方向 - 翻转
        WriteCmd(0xA0); // 段重新映射 - 翻转
    }
    else
    {
        // 正常显示模式
        WriteCmd(0xC8); // COM输出扫描方向 - 正常
        WriteCmd(0xA1); // 段重新映射 - 正常
    }
}

/**************************************************************
     Prototype      : void OLED_ShowFloat(int16_t x, int16_t y, float num, uint8_t int_len, uint8_t prec, uint8_t TextSize)
     Parameters     : x, y     -- 起始点坐标 (x: 0~127, y: 0~7)
                      num      -- 要显示的浮点数
                      int_len  -- 显示的总宽度（包含小数点与数字）
                      prec     -- 小数点后的位数
                      TextSize -- 字体大小(1:6x8点阵，2:8x16点阵)
     return         : none
     Description    : 显示格式化后的浮点数，自动转换为字符串并调用 OLED_ShowStr 显示
**************************************************************/
// 显示浮点数，使用 OLED_ShowStr 接口
void OLED_ShowFloat(int16_t x, int16_t y, float num, uint8_t int_len, uint8_t prec, uint8_t TextSize)
{
    char buf[32];
    snprintf(buf, sizeof(buf), "%*.*f", int_len, prec, num);
    OLED_ShowStr(x, y, (uint8_t *)buf, TextSize);
}